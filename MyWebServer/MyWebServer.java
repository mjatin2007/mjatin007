...
// Open the firefox browser and type "localhost:2540"
// The Web server will start working soon.


//Notes:
// I have tried to add as many types of file extrensions as possible inclusing the video file extension Mp4
//There might be exception error while running this program the first time. But it should be fine as soon as it runs the 2nd time.
//there were was an error while accessing the root folder, otherwise the programs works as expected.
//This problem persisted with MyListner as well but if ran the second time its better.
// I have also added the http streams to show what all was my output for various files
import java.io.*; 
...
//standard output to show the program is up and running
		...
		//while the modeswitch is true, which means if the server is running
		while(modeSwitch)
		{
